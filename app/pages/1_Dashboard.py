...
from services.market_router import (
    get_region, get_universe_names, get_universe_display_map,
    get_market_status, get_index_quotes, get_index_performance,
    get_sector_performance, fmt_currency, fmt_volume,
)
...
with h_col:
    flag = "🇺🇸" if is_us else "🇮🇳"
    st.markdown(f"## {flag} Market Dashboard")
with r_col:
...
@st.cache_data(ttl=300, show_spinner=False)
def _index_quotes(universe: str, rgn: str):
    return get_index_quotes(universe)

# ── Load core data ─────────────────────────────────────────────────────────────
with st.spinner("Loading market data…"):
    mkt_status = _market_status(region)
    idx_perf   = _index_perf(region)
...
if is_us:
    ...
    status_lbl = "● NYSE/NASDAQ OPEN" if is_open else "● NYSE/NASDAQ CLOSED"
    sp500      = mkt_status.get("sp500")
    change_pct = mkt_status.get("pct")
    vix        = mkt_status.get("vix")

    banner_parts = [f'<span style="color:{status_col};font-weight:700">{status_lbl}</span>']
    if sp500:
        try:
            gc = GREEN if float(change_pct or 0) >= 0 else RED
            banner_parts.append(
                f'<span style="color:{TEXT_DIM};margin-left:16px">S&P 500: '
                f'<span style="color:{gc};font-weight:600">'
                f'{float(sp500):,.1f} ({float(change_pct or 0):+.2f}%)</span></span>'
            )
        except Exception:
            pass
    if vix:
        try:
            vix_c = RED if float(vix) > 25 else (ORANGE if float(vix) > 18 else GREEN)
            banner_parts.append(
                f'<span style="color:{TEXT_DIM};margin-left:16px">VIX: '
                f'<span style="color:{vix_c};font-weight:600">{float(vix):.1f}</span></span>'
            )
        except Exception:
            pass
else:
    ...
    if mkt_cap:
        try:
            banner_parts.append(
                f'<span style="color:{TEXT_DIM};margin-left:16px">Mkt Cap: '
                f'<span style="color:{TEXT};font-weight:600">₹{float(mkt_cap):,.0f} L Cr</span></span>'
            )
        except Exception:
            pass
    if gift_nifty:
        try:
            ...
            banner_parts.append(
                f'<span style="color:{TEXT_DIM};margin-left:16px">GIFT Nifty: '
                f'<span style="color:{gc};font-weight:600">'
                f'₹{float(gift_nifty):,.1f} ({float(gift_pct or 0):+.2f}%)</span></span>'
            )
        except Exception:
            pass
    ...
    for col, item in zip(idx_cols, idx_perf[:n_cols]):
        ...
        with col:
            st.markdown(
                ...
                f'<div style="font-size:0.82rem;color:{ic};font-weight:600">'
                f'{sign} {float(chg):+,.1f} ({float(pct):+.2f}%)</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
...
# ═══════════════════════════════════════════════════════════════════════════════
# TABS — different sets for IN vs US
# ═══════════════════════════════════════════════════════════════════════════════
disp_map  = get_universe_display_map()
all_names = get_universe_names()
labels    = [disp_map.get(n, n) for n in all_names]

if is_us:
    tab_ov, tab_movers, tab_sector, tab_universe = st.tabs([
        "📈 Overview", "🚀 Top Movers", "🏭 Sectors", "🔍 Universe"
    ])
else:
...
# ── Default breadth index ─────────────────────────────────────────────────────
_breadth_index = "S&P 500" if is_us else "NIFTY 50"
...
with tab_ov:
    try:
        breadth_quotes = _index_quotes(_breadth_index, region)
    except Exception:
        breadth_quotes = []

    if breadth_quotes:
        adv  = sum(1 for s in breadth_quotes if (s.get("change_pct") or 0) > 0)
        dec  = sum(1 for s in breadth_quotes if (s.get("change_pct") or 0) < 0)
        unch = len(breadth_quotes) - adv - dec

        b1, b2, b3, b4, b5 = st.columns(5)
        with b1: st.metric("Total Stocks", len(breadth_quotes))
        with b2: st.metric("Advancing", adv, delta=f"{adv/max(len(breadth_quotes),1)*100:.0f}%")
        with b3: st.metric("Declining", dec, delta=f"-{dec/max(len(breadth_quotes),1)*100:.0f}%", delta_color="inverse")
        with b4: st.metric("Unchanged", unch)
        with b5:
            ad_ratio = adv / max(dec, 1)
            st.metric("A/D Ratio", f"{ad_ratio:.2f}",
                      delta="Bullish" if ad_ratio > 1.5 else ("Bearish" if ad_ratio < 0.7 else "Neutral"))

        total = adv + dec + unch
        if total > 0:
            fig_breadth = go.Figure()
            fig_breadth.add_trace(go.Bar(x=[adv], y=["Breadth"], orientation="h",
                                         name="Advancing", marker_color=GREEN,
                                         text=[f"▲ {adv}"], textposition="inside"))
            fig_breadth.add_trace(go.Bar(x=[unch], y=["Breadth"], orientation="h",
                                         name="Unchanged", marker_color=TEXT_DIM,
                                         text=[f"— {unch}"], textposition="inside"))
            fig_breadth.add_trace(go.Bar(x=[dec], y=["Breadth"], orientation="h",
                                         name="Declining", marker_color=RED,
                                         text=[f"▼ {dec}"], textposition="inside"))
        ...
        pct_vals = [s.get("change_pct") for s in breadth_quotes if s.get("change_pct") is not None]
        if pct_vals:
            fig_dist = go.Figure()
            fig_dist.add_trace(go.Histogram(x=pct_vals, nbinsx=20,
                                            marker_color=[GREEN if v >= 0 else RED for v in pct_vals]))
            index_label = "S&P 500" if is_us else "NIFTY 50"
            fig_dist.update_layout(
                title=f"{index_label} — Change % Distribution",
                ...
                yaxis=dict(title="Count", gridcolor=BORDER), bargap=0.05,
            )
            st.plotly_chart(fig_dist, use_container_width=True)

        # 52W extremes
        st.markdown(f'<div class="z-section">52-Week Extremes — {_breadth_index}</div>',
                    unsafe_allow_html=True)
        near_high = sorted([s for s in breadth_quotes if s.get("near_52w_high") is not None],
                           key=lambda x: float(x.get("near_52w_high") or 0), reverse=True)[:5]
        near_low  = sorted([s for s in breadth_quotes if s.get("near_52w_low") is not None],
                           key=lambda x: float(x.get("near_52w_low") or 0), reverse=True)[:5]

        wk_hi_col, wk_lo_col = st.columns(2)
        with wk_hi_col:
            st.markdown(f'<div style="color:{GREEN};font-weight:700;font-size:0.83rem;'
                        f'margin-bottom:6px">▲ NEAR 52W HIGH</div>', unsafe_allow_html=True)
            for s in near_high:
                pct = s.get("change_pct") or 0
                wh  = s.get("near_52w_high") or 0
                c   = GREEN if float(pct) >= 0 else RED
                price_str = fmt_currency(s.get("price") or 0)
                st.markdown(
                    f'<div class="z-stock-row">'
                    f'<span style="color:{TEXT};font-weight:700;min-width:100px">{s["symbol"]}</span>'
                    f'<span style="color:{TEXT_DIM};font-size:0.82rem">{price_str}</span>'
        ...
        with wk_lo_col:
            st.markdown(f'<div style="color:{RED};font-weight:700;font-size:0.83rem;'
                        f'margin-bottom:6px">▼ NEAR 52W LOW</div>', unsafe_allow_html=True)
            for s in near_low:
                pct = s.get("change_pct") or 0
        ...
        source_name = "Yahoo Finance" if is_us else "NSE API"
        st.info(f"Market overview data unavailable — {source_name} may be offline.")
...
with tab_movers:
    ...
    with mv_col3:
        sort_opts = ["Change %", "Volume", "Market Cap"] if is_us else ["Change %", "Volume", "30D Return", "365D Return"]
        sort_by = st.selectbox("Sort by", sort_opts, key="dash_mv_sort")
    ...
    if load_movers or st.session_state.get("dash_movers_data") is not None:
        if load_movers:
            with st.spinner("Fetching live movers…"):
                quotes = _index_quotes(mv_universe, region)
            st.session_state["dash_movers_data"]     = quotes
            st.session_state["dash_movers_universe"] = mv_universe
        else:
        ...
        if quotes:
            ...
            def _mover_row(s):
                ...
                row = {
                    "Symbol":   s["symbol"],
                    "Price":    fmt_currency(p),
                    "Change %": f"{float(chg):+.2f}%",
                    "Volume":   fmt_volume(vol) if vol else "—",
                    "_chg":     float(chg),
                }
                if not is_us:
                    row["30D Ret%"] = f"{float(s.get('return_30d') or 0):+.2f}%"
                else:
                    cap = s.get("market_cap")
                    from services.market_router import fmt_market_cap
                    row["Mkt Cap"] = fmt_market_cap(cap) if cap else "—"
                return row
            ...
            with gcol:
                st.markdown(f'<div style="color:{GREEN};font-weight:700;font-size:0.85rem;'
                            f'margin-bottom:6px">▲ TOP GAINERS ({mv_label})</div>',
                            unsafe_allow_html=True)
                st.dataframe(
            ...
            with lcol:
                st.markdown(f'<div style="color:{RED};font-weight:700;font-size:0.85rem;'
                            f'margin-bottom:6px">▼ TOP LOSERS ({mv_label})</div>',
                            unsafe_allow_html=True)
                st.dataframe(
            ...
            fig_mv = go.Figure(go.Bar(
                ...
                text=[f"{v:+.2f}%" for v in vals_chart],
            ...
            df_vol = pd.DataFrame([{
                "Symbol":   s["symbol"],
                "Price":    fmt_currency(s.get("price") or 0),
                "Volume":   fmt_volume(s.get("volume") or 0),
            ...
            st.warning("No data returned for this universe.")
    else:
    ...
    if load_sectors or st.session_state.get("dash_sector_data") is not None:
        if load_sectors:
            with st.spinner("Fetching sector performance…"):
                sector_data = _sector_perf(region)
        ...
        if sector_data:
            ...
            fig_tree = go.Figure(go.Treemap(
                ...
                customdata=df_sec[["pct", "last"]].values,
                hovertemplate=(
                    "<b>%{label}</b><br>Change: %{customdata[0]:+.2f}%<br>"
                    "Level: %{customdata[1]:,.2f}<extra></extra>"
                ),
                text=df_sec.apply(lambda r: f"{r['sector']}<br>{r['pct']:+.2f}%", axis=1),
                textinfo="text",
                marker=dict(
                    colors=df_sec["pct"],
                    colorscale=[[0, RED], [0.45, "#2a2e39"], [0.55, "#2a2e39"], [1, GREEN]],
                    cmid=0, showscale=True,
                    colorbar=dict(title=dict(text="Change %", font=dict(color=TEXT)),
                                  tickfont=dict(color=TEXT),
                                  bgcolor=CARD, bordercolor=BORDER),
                ),
            ))
            fig_tree.update_layout(paper_bgcolor=BG, font=dict(color=TEXT),
            ...
            table_cols = ["sector", "last", "pct"]
            if "advancing" in df_sec.columns:
                table_cols += ["advancing", "declining", "stocks"]
            elif "volume" in df_sec.columns:
                table_cols += ["volume"]
            df_sec_disp = df_sec[table_cols].copy()
            col_rename = {"sector": "Sector", "last": "Level", "pct": "Change %",
                          "advancing": "Advancing", "declining": "Declining",
                          "stocks": "Stocks", "volume": "Volume"}
            df_sec_disp = df_sec_disp.rename(columns=col_rename)
            ...
            st.dataframe(df_sec_disp.style.apply(_style_sec, axis=None),
                         use_container_width=True, hide_index=True)
            ...
            fig_bar = go.Figure(go.Bar(
                ...
                text=[f"{v:+.2f}%" for v in df_sec["pct"]], textposition="outside",
            ))
        ...
        if load_fii or st.session_state.get("dash_fii_data") is not None:
            ...
            if fii_rows:
                ...
                def _fii_card(col, label, val):
                    try:
                        fv = float(val)
                        col.metric(label, f"₹{abs(fv)/100:.1f}Cr",
                                   delta=f"{'+' if fv >= 0 else ''}{fv/100:.1f}")
                    except Exception:
                        col.metric(label, "—")

                st.markdown(f'<div class="z-section">Latest Day — {latest.get("date","")}</div>',
                            unsafe_allow_html=True)
                ...
                if not df_chart.empty:
                    ...
                    fig_fii.add_trace(go.Bar(
                        x=df_chart["date"], y=df_chart["fii_net"] / 100, name="FII Net (Cr)",
                        marker_color=[GREEN if v >= 0 else RED for v in df_chart["fii_net"]],
                    ))
                    fig_fii.add_trace(go.Bar(
                        x=df_chart["date"], y=df_chart["dii_net"] / 100, name="DII Net (Cr)",
                        marker_color=[BLUE if v >= 0 else ORANGE for v in df_chart["dii_net"]],
                    ))
                    fig_fii.update_layout(
                        title="FII & DII Net Activity — 30 Days", barmode="group",
                        template="plotly_dark", paper_bgcolor=CARD, plot_bgcolor=CARD,
                        height=340, margin=dict(l=10, r=10, t=40, b=30),
                        xaxis=dict(gridcolor=BORDER), yaxis=dict(gridcolor=BORDER, title="Net (₹ Cr)"),
                        legend=dict(orientation="h"),
                    )
                    st.plotly_chart(fig_fii, use_container_width=True)
            else:
                st.warning("FII/DII data unavailable — NSE API may be offline.")
        else:
            st.info("Click **Load FII/DII Data** to fetch institutional activity.")


# ─────────────────────────────────────────────────────────────────────────────
# TAB: Universe Explorer (quick view)
# ─────────────────────────────────────────────────────────────────────────────
_uni_tab = tab_universe
...
with _uni_tab:
    ...
    with u_col2:
        u_filter = st.text_input("Filter symbol", placeholder="e.g. AAPL, HDFC", key="dash_uni_filt")
    with u_col3:
        u_sort = st.selectbox("Sort by", ["Change %", "Price", "Volume"], key="dash_uni_sort")
    ...
    if load_uni or st.session_state.get("dash_uni_data_key") == u_universe:
        if load_uni:
            with st.spinner(f"Loading {u_label}…"):
                uni_quotes = _index_quotes(u_universe, region)
        ...
        if uni_quotes:
            sort_map = {"Change %": "change_pct", "Price": "price", "Volume": "volume"}
            ...
            st.markdown(f'<div class="z-section">{u_label} — {len(uni_quotes_s)} stocks</div>',
                        unsafe_allow_html=True)

            def _uni_row(q):
                row = {
                    "Symbol":   q["symbol"],
                    "Price":    fmt_currency(q.get("price") or 0),
                    "Change %": f"{float(q.get('change_pct') or 0):+.2f}%",
                    "High":     fmt_currency(q.get("high") or 0),
                    "Low":      fmt_currency(q.get("low") or 0),
                    "52W High": fmt_currency(q.get("year_high") or 0),
                    "52W Low":  fmt_currency(q.get("year_low") or 0),
                    "Volume":   fmt_volume(q.get("volume") or 0),
                    "_chg":     float(q.get("change_pct") or 0),
                }
                if not is_us:
                    row["30D Ret%"]  = f"{float(q.get('return_30d') or 0):+.2f}%"
                    row["365D Ret%"] = f"{float(q.get('return_365d') or 0):+.2f}%"
                return row

            df_uni = pd.DataFrame([_uni_row(q) for q in uni_quotes_s])
            ...
            disp_cols = [c for c in df_uni.columns if not c.startswith("_")]
            st.dataframe(
                df_uni[disp_cols].style.apply(_style_uni, axis=None),
                use_container_width=True, hide_index=True, height=500,
            )
            st.download_button(
                "📥 Export CSV", df_uni[disp_cols].to_csv(index=False),
                f"{u_universe.replace(' ', '_')}_snapshot.csv", "text/csv",
            )
        else:
            st.warning("No data returned for this universe.")
    else:
        st.info("Select a universe and click **Load Universe** to see all stocks.")
...
if auto_refresh:
    ...
    for i in range(60, 0, -1):
        placeholder.markdown(
            ...
            f'Auto-refresh in {i}s…</div>', unsafe_allow_html=True,
        )
        time.sleep(1)
    st.cache_data.clear()
    st.rerun()
