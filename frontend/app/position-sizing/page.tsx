...
import { formatCurrency, formatNum } from "@/lib/utils";
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        >
          {([
            [`Capital (${currencySymbol})`, "capital", "number"],
            ["Risk per trade (%)", "risk_pct", "number"],
            ["Max position (%)", "max_position_pct", "number"],
            [`Entry (${currencySymbol})`, "entry", "number"],
            [`Stop loss (${currencySymbol})`, "stop", "number"],
            [`Target (${currencySymbol}) — optional`, "target", "number"],
        ...
        {m.data && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <KPI label="Capital deployed" value={formatCurrency(m.data.deployed, region)} />
            <KPI label="At risk" value={formatCurrency(m.data.at_risk, region)} trend="down" />
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <form onSubmit={(e) => { e.preventDefault(); m.mutate(); }} className="grid md:grid-cols-3 gap-3">
          {([
            [`Capital (${currencySymbol})`, "capital"],
        ...
        {m.data && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <KPI label="Deploy" value={formatCurrency(m.data.deploy_amount, region)} />
